...
class FirebaseTodoAPI {
  ...
  Future<String> addTodo(Map<String, dynamic> todo) async {
    try {
      final docRef = await db.collection("todos").add(
          todo); //we couldn’t delete coz there’s no stored id so we need to update the id stored.
      await db
          .collection("todos")
          .doc(docRef.id)
          .update({'id': docRef.id}); //only need one of this
  ...
  Future<String> toggleStatus(String? id, bool status) async {
    try {
      ...
      return "Successfully updated todo status!";
    } on FirebaseException catch (e) {
      return "Failed with error '${e.code}: ${e.message}";
    }
  }
